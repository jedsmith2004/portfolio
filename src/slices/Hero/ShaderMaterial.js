...
export const vertexShader = /* glsl */ `
  varying vec2 vUv;
  void main() {
    vUv = uv;  // For a plane, the built-in UVs are fine.
...
Model hexModel(
    ...
    // Define your palette of 6 colors (normalized RGB values)
    const vec3 palette[6] = vec3[6](
    vec3(0.043, 0.035, 0.039), // 0B090A
    vec3(0.086, 0.102, 0.114), // 161A1D
    vec3(0.400, 0.027, 0.031), // 660708
    vec3(0.643, 0.086, 0.102), // A4161A
    vec3(0.729, 0.094, 0.106), // BA181B
    vec3(0.898, 0.220, 0.231)  // E5383B
    );

    // Calculate t from your existing expression.
    float t = fract(dot(hexCenter, pca) * 5.0 + length(p) + 0.8);
    // Scale t to the range [0, 5] (since there are 5 transitions between 6 colors)
    float scaledT = t * 5.0;
    float idx = floor(scaledT);
    float f = fract(scaledT);

    // Ensure we don't go out of bounds. When idx is 5, we stay at palette[5].
    vec3 edgeColor = mix(palette[int(idx)], palette[int(min(idx+1.0, 5.0))], f);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Use the plane's UVs (vUv) so that the effect is centered:
    // vUv ranges from 0 to 1; subtract 0.5 to center it, then multiply by 2 to map to [-1,1].
    vec2 p = (vUv - vec2(0.5)) * 2.0;
    // Scale x by the inverse of the aspect ratio to "unsquish" horizontally.
    p.y *= iResolution.y / iResolution.x * 1.1;
    float effectScale = 6.0; // adjust this to control overall size
    p *= effectScale;
    ...
    // Create camera matrix and view ray
    ...
    float alpha = hit.isBackground ? 0.0 : 1.0;
    fragColor = vec4(color, alpha);
}
    
// The main function calls mainImage and outputs its result.
void main() {
  vec4 color;
